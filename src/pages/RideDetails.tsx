...
export default function RideDetails() {
  ...
  const [showDriverEmail, setShowDriverEmail] = useState(false)
  ...
  const canContact = !isDriver && (isPassenger || (!noSeats && !mutation.isSuccess))
    ...
    <div className=" bg-[#F3F4F6] py-8 px-4">
      <div className="max-w-md mx-auto">
        ...
        <div className="flex gap-3">
          ...
          <button
            type="button"
            onClick={() => setShowDriverEmail(true)}
            hidden={!canContact || mutation.isPending}
            className="w-12 h-12 bg-[#1A365D] text-white rounded-xl flex items-center justify-center hover:bg-[#162d52] transition-colors shrink-0"
        ...
        {showDriverEmail && (
          <EmailModal
            tripId={Number(id)}
            recipientProfilId={trip.driver.profilId}
            recipientName={`${trip.driver.firstname} ${trip.driver.lastname}`}
            onClose={() => setShowDriverEmail(false)}
          />
        )}
      </div>
    </div>
